...
fn read_sysfs(path: &'static str) -> Result<u32, String> {
    ...
    if f.is_err() {
        return Err(format!("Couldn't open '{}' because '{}'", path, f.unwrap_err()));
    }
    let mut s = String::new();
    if let Err(err) = f.unwrap().read_to_string(&mut s) {
        return Err(format!("Couldn't read from '{}' because '{}'", path, err));
    }
    s.trim_right().parse::<u32>()
        .and_then(|v| { Ok(v) })
...
fn write_sysfs(path: &'static str, value: u32) -> Result<(), String> {
    ...
    f.unwrap().write_fmt(format_args!("{}", value))
        .or_else(|err| {
...
fn current() -> Result<u32, String> {
    maximum()
        .and_then(|max| {
            read_sysfs(BRIGHTNESS)
                .and_then(|v| { Ok(v *100 / max) })
        })
}
...
fn main() {
    ...
    result.unwrap_or_else(|err| {
        eprintln!("Could run command: '{}' because '{}'", config.command, err);
        usage().unwrap();
        process::exit(1);
    });
}
